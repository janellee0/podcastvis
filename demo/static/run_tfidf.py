...
def generate_tfidf(data, corpus):
	...
	beg_of_doc = tiles[0].index(tiles[0])
	for i in range(len(tiles)):
		#print "tile " + str(i)
		scores = map(lambda x: (my_dictionary[x[0]], x[1]), corpus_tfidf[i])
		#print sorted(scores, key=lambda x:x[1], reverse=True)
		for m in re.finditer(ur'(\w|\'\w)+', tiles[i], re.UNICODE):
			start, end = m.span()
			word = m.group().encode('utf-8')
			score = next((x[1] for x in scores if x[0] == word.lower()), 0)
			scored_transcript.append({
